...
const targetScore = parseInt(localStorage.getItem('targetScore'), 10);
...
function setup() {
  ...
  if (!playerName || !roomName || !targetScore) {
    alert('Missing info. Please sign in again.');
    window.location.href = 'signin.html';
    return;
  }

  // Use this data to start the game
  document.getElementById('team-info').textContent = `Jugador: ${playerName} | Sala: ${roomName}`;
  document.getElementById('scoreboard').textContent = `Puntaje objetivo: ${targetScore}`;

  // Example match stats (you'll update this during gameplay)
  window.matchStats = {
    players: [
      { name: playerName, wins: 0, losses: 0 }
    ]
  };
});
